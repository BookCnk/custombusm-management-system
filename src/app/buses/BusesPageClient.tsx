...
export default function BusesPageClient({
  initialBuses,
}: {
  initialBuses: BusData[];
}) {
  ...
  const [copyBusNumber, setCopyBusNumber] = useState("");
  ...
  const updateLayoutField = (
    ...
    setFormData((prev) => {
      const currentLayout =
        prev.layout || createDefaultSeatLayout(prev.totalSeats || 40);
      const nextLayout = {
        ...currentLayout,
        [field]: value,
      } as SeatLayout;
  ...
  const handleDuplicateClick = (bus: BusData) => {
    setSelectedBus(bus);
    setCopyBusNumber(`${bus.busNumber}-copy`);
  ...
  const handleConfirmCopy = async () => {
    if (!selectedBus || !copyBusNumber.trim()) return;

    try {
      await apiRequest("/api/buses", {
        method: "POST",
        body: JSON.stringify({
          busNumber: copyBusNumber.trim(),
          type: selectedBus.type,
          status: selectedBus.status,
          totalSeats: selectedBus.totalSeats,
          layout: selectedBus.layout,
        }),
      });
      await loadBuses();
              ...
              <div
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={() => void handleDuplicateClick(bus)}
                    ...
                    <button
                      onClick={() => void handleDeleteClick(bus)}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden">
            <div className="text-center p-6">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Copy size={32} className="text-green-600" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden">
            <div className="text-center p-6">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                คุณต้องการลบรถบัสนี้?
              </h3>
              <p className="text-gray-500 mb-2">{selectedBus?.busNumber}</p>
              <p className="text-sm text-gray-400 mb-6">
                {selectedBus?.type} - {selectedBus?.totalSeats} ที่นั่ง
              </p>
              <div className="flex gap-3">
                <button
                  type="button"
